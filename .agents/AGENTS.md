...
Antigravity project skills are defined in `.agents/skills/`:
...
- For a new Flutter feature, enhancement, migration session, architectural change, or substantial explanation, delegate the bounded work using the `flutter_architecture_expert` skill.
- The architecture expert must inspect requirements, perform the reuse/package check, share a plan, and wait for explicit user approval before implementing features or enhancements.
- A truly simple localized bug may be fixed directly under the architecture expert's narrow bug-fix rules.
- After each migration/development session, or whenever the user asks for a review, trigger a separate review using the `flutter_session_reviewer` skill after implementation stops.
